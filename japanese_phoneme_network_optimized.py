...
class NeuralNetwork():
	...
	def genInputActivations(self):
		for file_ in glob.glob("wavelets_easy/*"):	
			...
			self.inputActivations.append((activation,file_[14:-4]))
			...
			for hidden in self.hiddenLayer:
				...
				for input_ in self.inputLayer:
					#the change in weights for each input node are:
						...
						#multiplied by y(1-y) <-- the derivative of the unit's activation function!
					backprop_counter = 0
					tw = 0
					zw = 0
	...
	def saveWeights(self):
		...
		weights = open("weights_12_new.data","w")
